...
sonuc_json =json.loads(sonuc.text) #dict olarak pythona çeviyoruz.
...
response = requests.get("https://horoscope-app-api.vercel.app/api/v1/get-horoscope/daily?sign=pisces&day=today")
...
with open("horoscope.json", "w") as file:
    json.dump(data, file)
print("Başarıyla Kaydedildi!")

#how to retrieve data from a json file

with open("horoscope.json", "r") as file:
    data = json.load(file)
...
tarih = data["data"]["date"]
print(tarih)

fal = data["data"]["horoscope_data"]
print(f"Bugün {tarih} ve falınız: {fal}")
...
#Create a python Dict from JSON
...
class PersonEncoder(json.JSONEncoder):
    def default(self, obje):
        if isinstance(obje, Person):
            return {"name": obje.name, "age": obje.age, "horoscope": obje.horoscope}
        return super().default(obje)
    
#create a person

person = Person("Toygar Par", 50, "Pisces")    

#encode the custom obje using the custom encoder
...
Python'dan JSON2a dönüşen değerler
